    ...
    m_ui(new Ui::ViewDialog), trn(t)
{
    ...
    m_ui->lstViews->addAction(QWhatsThis::createAction(this));
}
...
void ViewDialog::on_cmdAdd_clicked()
{
    ...
    foreach (Club c, trn->getClubs()){
        if (texts.contains(c.displayName)) {
           ...
           items = m_ui->lstTeams->findItems(c.displayName, Qt::MatchCaseSensitive);
           m_ui->lstTeams->removeItemWidget(items.at(0));
        }
    }
...
void ViewDialog::on_pushButton_clicked()
{
    foreach (View v, trn->views())
        ...
        m_ui->lstViews->removeItemWidget(m_ui->lstViews->currentItem());
                trn->removeView(v);
...
void ViewDialog::on_lstViews_currentItemChanged(QListWidgetItem* current, QListWidgetItem* previous)
{
    {
    foreach (View v, trn->views())
